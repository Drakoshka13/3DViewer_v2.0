...
void MyOpenGLWidget::PaintObj() {
    ...
    glPointSize(obj.size_p);
        ...
        if(obj.disable_line == false){
            glDrawElements(GL_LINES, obj.data.f_count, GL_UNSIGNED_INT, &obj.data.facets[0]);
        }
...
void MyOpenGLWidget::ProjectionParallel() {
    glOrtho(obj.data.max_min_xyz[0][1] * 1.5f, obj.data.max_min_xyz[0][0] * 1.5f, obj.data.max_min_xyz[1][1] * 1.5f, obj.data.max_min_xyz[1][0] * 1.5f,
...
void MyOpenGLWidget::ProjectionCenter() {
    ...
    glFrustum(obj.data.max_min_xyz[0][1], obj.data.max_min_xyz[0][0], obj.data.max_min_xyz[1][1], obj.data.max_min_xyz[1][0], heapHeight, obj.data.max_min_xyz[2][0] * 100);
